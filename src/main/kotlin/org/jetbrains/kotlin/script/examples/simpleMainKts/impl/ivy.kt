...
class IvyResolver : ExternalDependenciesResolver {
    ...
    private fun resolveArtifact(
        ...
        val diagnostics = report.allProblemMessages.map { (it as String).asErrorDiagnostics() }

        return if (report.hasError()) {
            makeFailureResult(diagnostics)
        } else {
            report.allArtifactsReports.map { it.localFile }.asSuccess(diagnostics)
        }
    }
